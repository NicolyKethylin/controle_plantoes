...
## ListHospitals
You can execute the `ListHospitals` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
listHospitals(): QueryPromise<ListHospitalsData, undefined>;
...
You can also pass in a `DataConnect` instance to the action shortcut function or `QueryRef` function.
```typescript
listHospitals(dc: DataConnect): QueryPromise<ListHospitalsData, undefined>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the listHospitalsRef:
```typescript
const name = listHospitalsRef.operationName;
console.log(name);
```

### Variables
The `ListHospitals` query has no variables.
### Return Type
Recall that executing the `ListHospitals` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `ListHospitalsData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `ListHospitals`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, listHospitals } from '@dataconnect/generated';


// Call the `listHospitals()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
...
### Using `ListHospitals`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, listHospitalsRef } from '@dataconnect/generated';


// Call the `listHospitalsRef()` function to get a reference to the query.
const ref = listHospitalsRef();

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = listHospitalsRef(dataConnect);
...
// Variables can be defined inline as well.
const { data } = await createShift({ hospitalId: ..., shiftTypeId: ..., userId: ..., date: ..., durationInMinutes: ..., startTime: ..., endTime: ..., notes: ..., });
...
// Variables can be defined inline as well.
const ref = createShiftRef({ hospitalId: ..., shiftTypeId: ..., userId: ..., date: ..., durationInMinutes: ..., startTime: ..., endTime: ..., notes: ..., });
